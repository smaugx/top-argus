...
class PacketInfo(Bean):
    ...
    def to_dict(self,timeformat = False):
        ...
        case['id']               = self.id
        case['gid']              = self.gid
        case['service_name']     = self.service_name
        case['type']             = self.type
        case['name']             = self.name
        case['priority']         = self.priority
        case['note']             = self.note
        case['total_count']      = self.total_count
        case['finished_count']   = self.finished_count
        case['unfinished_count'] = self.unfinished_count
        case['fresh_count']      = self.fresh_count
        case['timestamp']        = self.timestamp
        case['update_at']        = self.update_at
        if timeformat:
            case['timestamp'] = int(time.mktime(time.strptime(str((self.timestamp)),'%Y-%m-%d %H:%M:%S')))
            case['update_at'] = int(time.mktime(time.strptime(str((self.update_at)),'%Y-%m-%d %H:%M:%S')))
  ...
  def update_case(cls,data=None, where=''):
    print 'update table main_event_cases: %s where:%s' % (json.dumps(data),where)
